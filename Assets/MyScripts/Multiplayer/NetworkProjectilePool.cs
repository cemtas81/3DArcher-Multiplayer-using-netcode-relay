...
{
    private static NetworkProjectilePool _instance;

    public static NetworkProjectilePool Singleton { get { return _instance; } }
    ...
    public void OnValidate()
    {
        for (var i = 0; i < PooledPrefabsList.Count; i++)
        {
            ...
            if (prefab != null)
            {
                Assert.IsNotNull(prefab.GetComponent<NetworkObject>(), $"{nameof(NetworkProjectilePool)}: Pooled prefab \"{prefab.name}\" at index {i.ToString()} has no {nameof(NetworkObject)} component.");
            }
        }
    }
...
{
    GameObject m_Prefab;
    NetworkProjectilePool m_Pool;

    public PooledPrefabInstanceHandler(GameObject prefab, NetworkProjectilePool pool)
    {
        m_Prefab = prefab;
        m_Pool = pool;
    }
